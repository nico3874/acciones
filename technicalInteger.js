...
function determinarTendenciaMensual(datosHistoricos) {
  const cierres = datosHistoricos.map(entry => entry.close);

  const tendenciaMensual =
    cierres[0] < cierres[cierres.length - 1] ? 'Alcista' : 'Bajista';

  return tendenciaMensual;
}

async function analizarAcciones(simbolos) {
  for (const symbol of simbolos) {
    const startDate = '2023-01-01';
    const endDate = new Date().toISOString().split('T')[0];
    const fechaPrediccion = '2024-02-11';
    ...
    if (datosHistoricos.length > 0) {
      ...
      const tendenciaMensual = determinarTendenciaMensual(datosHistoricos);

      console.log(`Análisis para ${symbol}:`);
      console.log(`- Valor predicho para ${fechaPrediccion}: ${valorPredicho}`);
      console.log(`- Señal de compra: ${senalCompra}`);
      console.log(`- Señal de venta: ${senalVenta}`);
      console.log(`- Tendencia mensual: ${tendenciaMensual}`);
      console.log('---');
    } else {
...
// Ejemplo de uso con varias acciones
